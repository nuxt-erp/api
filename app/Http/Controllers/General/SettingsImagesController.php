...
class SettingsImagesController extends ControllerService
{

    protected $repository;
    protected $resource;

    public function __construct(SettingsImagesRepository $repository, SettingsImagesResource $resource)
    {
    ...
    public function store(Request $request)
    {
        $user = auth()->user();
        $files = [];
        if ($request->hasFile('files') && $request->filled('type')) {
            $x = 0;
            ...
            foreach ($request->file('files') as $file) {
                ...
                // save original file
                $path = $file->store('company_'.$user->company_id.'/'.$request->type.'/images', ['disk' => 's3']);
