                            ...
                            if ($attributes[$i]!=0)
                            {
                                ...
                                if ($product_id) {
                                    ...
                                    if ($qty_received > 0) {
                                        ...
                                        // $updateStock = new AvailabilityRepository(new Availability());
                                       // $updateStock->updateStock($product_id, $qty_received, $data["location_to_id"], "+", "Transfer", $id,0,0, "Receiving quantity");

                                        // Decrement stock from sender
                                       // $updateStock->updateStock($product_id, $qty_received, $data["location_from_id"], "-", "Transfer", $id, 0,0, "Sending quantity");
            ...
            if(isset($getItem[0]->details)){
                foreach ($getItem[0]->details as $value)
                {
                    if ($value->qty_received > 0) { // IF ALREADY RECEIVED
                        //$updateStock = new AvailabilityRepository(new Availability());
                       // $updateStock->updateStock($value->product_id, $value->qty_received, $getItem[0]->location_to->id, "-", "Transfer", $id, 0,0, "Undo transfer item - receiver");    // DECREMENT STOCK FROM RECEIVER LOCATION
                       // $updateStock->updateStock($value->product_id, $value->qty_received, $getItem[0]->location_from->id, "+", "Transfer", $id, 0,0, "Undo transfer item - sender");  // INCREMENT STOCK FROM SENDER LOCATION
                    }
                }
            }
